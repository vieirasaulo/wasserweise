...
Get = ww.queries.Get(database_fn)
...
date_wid = pn.widgets.DatetimePicker(
    name="Date and time", start=start, end=end, value=value
)
wwalearrows_wid = pn.widgets.IntSlider(
    start=1, end=100, step=1, value=10, name="Sizing arrows"
)
...
# HydroProfile
drills_wid = pn.widgets.Select(
    name="Drill Name", options=list(HydroProfile_df.DrillName.unique())
)
...
g_df = (
    df.groupby(["Name", pd.Grouper(freq="D")])["Value"].mean().to_frame().reset_index()
)
gr_df = (
    r_df.groupby(["MonitoringPointName", pd.Grouper(freq="D")])["Value"]
    .mean()
    .to_frame()
    .reset_index()
)
...
# iwwatterTS = iBindTS.hvplot.wwatter(
#     x = 'Date', y = 'Value',
#     label = 'Diver Data',
#     # width = max_width,
...
# wwatter_rg = gr_df.hvplot.wwatter(
#     x='Date', y='Value',
#     ylabel = '[m]',
#     xlabel = 'Date',
#     size = 50,
...
# iwwatterTS =  wwatter_rg * iwwatterTS


def iTS(wells_wid):

    g1_df = g_df[g_df.Name == wells_wid].reset_index(drop=True)

    maxy, miny = 115, 107
    iwwatterTS = g1_df.hvplot.wwatter(
        x="Date",
        y="Value",
        label="Diver Data",
        # width = max_width,
...
iwwatterTS = pn.bind(iTS, wells_wid)
...
def iPlot(date_wid, wwalearrows_wid):

    try:

        map_gdf, river_gage_gdf = ww.utils.prepare_query(Get, date_wid)

        grid_x_gcs, grid_y_gcs, grid_z_gcs, U, V = ww.utils.Interpolation_Gradient(
            map_gdf, crs_utm=25833, pixel_size=10
        )

        arrows_df = ww.utils.arrow_head(
            grid_x_gcs, grid_y_gcs, grid_z_gcs, U, V, wwale=wwalearrows_wid
        )
        df = arrows_df[
            arrows_df.index.isin(np.arange(0, arrows_df.shape[0], 2))
        ].reset_index(drop=True)

        Map = ww.utils.Folium_map(Get)

        Map_contour = ww.utils.Folium_contour(
            Map, map_gdf, river_gage_gdf, grid_x_gcs, grid_y_gcs, grid_z_gcs
        )

        # arrows_df
        Map = ww.utils.Folium_arrows(Map_contour, df)
...
iMap = pn.bind(iPlot, date_wid, wwalearrows_wid)

# Aquifer Characterization


def iHPV(drills_wid):
    df = plot_df[(plot_df.DrillName == drills_wid)].reset_index(drop=True)
...
def iHPL(drills_wid):
    ...
    df_ = layers_df[(layers_df.Drill == drills_wid)].reset_index(drop=True)
...
### Map
Map_r1 = pn.Row(date_wid, wwalearrows_wid)
Map_r2 = pn.Row(iMap)
Map_c = pn.Column(
    Map_r1,
    Map_r2,
...
AC_c = pn.Column(AC_r1, AC_r2, min_width=min_width, max_width=max_width)
