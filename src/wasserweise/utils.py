...
def DbCon(database_fn: str):
    ...
    engine = create_engine(
        "sqlite:///{}".format(database_fn), echo=False
    )  # False to not show the output
...
def CompleteMissingDates(df: pd.core.frame.DataFrame):
    ...
    df.MonitoringPointID = df.MonitoringPointID.fillna(
        [i for i in df.MonitoringPointID.unique() if isinstance(i, np.int64)][0]
    )
    df.VariableID = df.VariableID.fillna(
        [i for i in df.VariableID.unique() if isinstance(i, np.int64)][0]
    )
...
def Process(df: pd.core.frame.DataFrame, Get_: wasserweise.queries.Get):
    """
...
def LinearInterpolation(
    DataFrame: pd.core.frame.DataFrame, Longitude: str, Latitude: str
):
...
class PrepareIsolines(wasserweise.queries.Get):
    """
    ...
    def DataFrame(self, date_wid):
        ...
        if (
            df[~df.Value.isna()].shape[0] == 0
        ):  # if every value is na we fill everything
    ...
    def DataSet(self):
        ...
        ds = xr.Dataset(
            ...
            coords=dict(Lon=(["x", "y"], grid_x), Lat=(["x", "y"], grid_y)),
        )
        return ds
...
def prepare_query(Get_: wasserweise.queries.Get, date_wid, crs_gcs: int = 4326):
    """
    ...
    if df.shape[0] < 3:
        ...
        try:
            ...
            map_gdf = gpd.GeoDataFrame(
                map_df, geometry=gpd.points_from_xy(map_df.E, map_df.N), crs=crs_gcs
            ).reset_index(drop=True)
            river_gage_gdf = gpd.GeoDataFrame(
                river_gage_df,
...
def Interpolation_Gradient(
    map_gdf: gpd.geodataframe.GeoDataFrame, crs_utm: int = 25833, pixel_size: int = 20
):
    """
    Function to interpolate data the cubic method and find the gradient of the potentiometric surface.
    ...
    try:
        ...
        points_utm = np.array(
            pd.DataFrame({"x": map_gdf.geometry.x, "y": map_gdf.geometry.y})
        )
        ...
        grid_z_utm = griddata(
            points_utm, values, (grid_x_utm, grid_y_utm), method="cubic"
        )
...
def arrow_head(
    ...
    try:
        ...
        df["rotation"] = [
            geodesic.inv(pair[0][1], pair[0][0], pair[1][1], pair[1][0])[0] + 90
            for pair in pairs
        ]
...
def Folium_contour(
    ...
    if grid_x is None:
        ...
        contour = plt.contour(
            grid_x, grid_y, grid_z, linewidths=2, colors="steelblue", levels=levels
        )
        ...
        points_line = isolines_gdf.apply(
        ...
        points_gdf = map_gdf[
            map_gdf.MonitoringPointName != "RG"
        ]  # deleting fictious points
        ...
        try:
            ...
            for row in isolines_gdf.iterrows():
                ...
                line = row.lines
                folium.PolyLine(
                    line, color=row.stroke, weight=3, popup=f"""{row['level-value']}m"""
                ).add_to(m)
...
def Folium_arrows(
    m: folium.folium.Map, arrows_df: pd.core.frame.DataFrame, sample_size=20
):
    ...
    try:
        ...
        for i in range(df.shape[0]):
            ...
            folium.PolyLine(locations=coordinates, weight=2, color="black").add_to(m)

            folium.RegularPolygonMarker(
                location=(df["head_y"][i], df["head_x"][i]),
                color="black",
                fill=True,
                fill_color="black",
                fill_opacity=1,
...
def HydraulicGradient(Get_: wasserweise.queries.Get, size: int = 2000):
    ...
    for t in times_sample:
        ...
        if Get_.Isolines_df.shape[0] > 3:
            ...
            try:
                ...
                grid_x, grid_y, grid_z, U, V = Interpolation_Gradient(
                    map_gdf, crs_utm=25833, pixel_size=10
                )

                df = (
                    arrow_head(grid_x, grid_y, grid_z, U, V)
...
wasserweise_fn = "Assets/wasserweise_white.png"
wasserweise_fn_ = "Assets/wasserweise.png"
