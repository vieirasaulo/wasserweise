...
import time
import wasserweise.api as api
import wasserweise.utils as utils
from wasserweise.api import PegelAlarm
from wasserweise.api import Inowas
...
def Update(Process_df, Get_):
    ...
    if Process_df is not None:
        Process_df.to_sql(
            name="PointsMeasurements",
            con=Get_.connection,
            if_exists="append",
            index=False,
        )
        print(
            f"Diver data updated from ID {Process_df.iloc[0,0]} to ID {Process_df.iloc[-1,0]}"
        )
    else:
...
def SequenceUpdate(sensor, sts, Get_):
    with open(fn, "a+") as f:
        ...
        variables = Sensors_df[
            Sensors_df.Diver == sensor
        ].VariableName  # indexing variables of interest
...
def InowasLongAPItoSQL(Get_):
    ...
    FunctioningDivers_df = DiversNextUpdate_df[
        (DiversNextUpdate_df.IOT == 1) & (DiversNextUpdate_df.Functioning == 1)
...
def app_update(git_commit: bool, github_push: bool):
    ...
    Get = wasserweise.queries.Get(database_fn)
    ...
    if git_commit:
        ...
        database_list = [
            "Data/" + file for file in contents if "db" in file or "LOG" in file
        ]
    ...
    if github_push:
        ...
        database_list = [
            file for file in contents if "db" in file.path or "LOG" in file.path
        ]

        for i in range(len(database_list)):
            repo.update_file(
                database_list[i].path,
                commit_message,
                commit_message,
                database_list[i].sha,
            )
