...
class Get:
    def __init__(self, database_fn: str):
        ...
        engine = create_engine(
            "sqlite:///{}?check_same_thread=False".format(database_fn),
    ...
    def APIDate(self, MonitoringPointID: int):
        ...
        try:
            ...
            first_t = last_t + timedelta(
                hours=1
            )  # first time - adding an hour to last time saved
    ...
    def CheckDuplicateEntry(
        self, MonitoringPointID: int, VariableID: int, df: pd.core.frame.DataFrame
    ):
        """
        ...
        check_df["keys"] = (
            check_df.MonitoringPointID.astype("str")
            + "_"
            + check_df.TimeStamp.astype("str")
            + "_"
            + check_df.VariableID.astype("str")
        )
        df["TimeStamp"] = df["TimeStamp"].astype("int64")
        df["keys"] = (
            df.MonitoringPointID.astype("str")
            + "_"
            + df.TimeStamp.astype("str")
            + "_"
            + df.VariableID.astype("str")
        )
    ...
    def Isolines(self, Year, Month, Day, Hour):
        ...
        if Runs_key not in self.Isolines_Runs:
            ...
            DataFrame = pd.read_sql(query, con=self.connection)
            DataFrame = DataFrame[
                DataFrame.VariableID.isin([0, 7])
            ]  # indexing head and raingage
    ...
    def Isolines_debug(self, Year, Month, Day, Hour):
        ...
        if Runs_key not in self.Isolines_Runs:
            ...
            # indexing columns
            DataFrame = DataFrame[cols]
            DataFrame["DiverReading"] = np.round(
                DataFrame.Value - DataFrame.CaseTop + DataFrame.DiverDepth, 2
            )
            n = 2
            ...
            DataFrame = DataFrame.round(
                {"CaseTop": n, "DiverDepth": n, "Value": n, "DiverReading": n}
            )
            ...
            if DataFrame.duplicated(subset=["MonitoringPointName"]).any():
                ...
                v = (
                    DataFrame.groupby("MonitoringPointName")
                    .agg({"Value": [nanmean]})
                    .reset_index()
                )
    ...
    def HydroProfile(self):
        ...
        if "HydroProfile" in self.HydroProfile_Runs:
            ...
            self.HydroProfile_Runs[
                "HydroProfile"
            ] = DataFrame  # store in class variable
